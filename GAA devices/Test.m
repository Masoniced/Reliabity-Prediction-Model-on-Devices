...
clear
clc
load('C:\Users\Mason\Documents\Project\Matlab Project\MMC-GAA\Byproducts\190_50_11_GAA_PIT2.mat')
Data=load('C:\Users\Mason\Documents\Project\Matlab Project\Clustering data processing\Fourier\current.mat');
D=Data.res;
chose_x=5;
chose_y=3;
...
x=[28.001564057191793, -19.955148304698586];
x_r=[32.8248319292306;-37.0387213298481];
slope=0.8588;
Ipre_mean_variation_factor=0.2448;

Initial_Rp=((VI*4e-9/((n3-2)*aphla_C))/((VI*4e-9/((n3-2)*aphla_C))^(x_r(1))*exp(x_r(2))))*(n_w*n_l*n_f)/t_o;
Initial_Ipre_mean=((VI*4e-9/((n3-2)*aphla_C))^(x_r(1))*exp(x_r(2)))*2.4675e4; % (t_o/(n_w*n_l*n_f))=2.4675e4
...
while criteria==1
    ...
    for i=1:mini_batch
        ...
        delt_time(i)=(log(1/(seed(i,3)^3)))*(beta)*(n1*n2*aphla_C^2/(n_w*n_l*n_f))^(3/log(log(sum(total_rate(:)))))*((VI*4e-9/((n3-2)*aphla_C))^(v_factor_1)*exp(v_factor_2)*1.7)*(4e-10/aphla_C);
...
%     if I>Result_list(1,3)*scale*3
    if counter<=15
        ...
        if (I/I_list(end-1)>1.5)&&(length(I_list)>5)
            criteria=0;
            fprintf('\n Breakdown')
        end
    end
...
s=Result_list(:,3)/Result_list(3,3)*Initial_Ipre*(n_w*n_l*n_f)/t_o*(Result_list(2,1))^(slope)*(exp(x(1)*log((VI*4e-9/((n3-2)*aphla_C)))+x(2)))/1.4;
