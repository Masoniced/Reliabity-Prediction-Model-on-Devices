function [p,data,final,curve]=Cluster_fitting_clusterweibull(Data,ratio,fixa,fixb,fixtor)
syms a2 b1 tor1 b2 tor2 t
CDF=1-((1+1/a2*(t/tor2)^b2)^(-a2))*exp(-(t/tor1)^(b1));
PDF=diff(CDF,t);
LF=-log(PDF);
...
data=sort(data);
x0=[0.001,1,1,data(round(length(data)*0.63*ratio)),data(round(length(data)*0.63*(1-ratio)+length(data)*ratio))];
%x0=[1,1,5];
alph=5;
% fa1=diff(LF,a1);
fb1=diff(LF,b1);
...
if isempty(fixa)&&isempty(fixb)&&isempty(fixtor)
    for i=1:length(data)
%         t_G1=G1(a1,a2,b1,b2,data(i),tor1,tor2)+t_G1;
        t_G2=G2(a2,b1,b2,data(i),tor1,tor2)+t_G2;
        t_G3=G3(a2,b1,b2,data(i),tor1,tor2)+t_G3;
        t_G4=G4(a2,b1,b2,data(i),tor1,tor2)+t_G4;
        t_G5=G5(a2,b1,b2,data(i),tor1,tor2)+t_G5;
        t_G6=G6(a2,b1,b2,data(i),tor1,tor2)+t_G6;        
        F=Lf(a2,b1,b2,data(i),tor1,tor2)+F;
    end
%     t_G1=matlabFunction(t_G1);
    ...
    error=1;
    c=1;
    F1=F(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5));
    count=0;
    list=[];
    while error>criteria
        count=count+1;
        list(count,1:5)=temp_x;
        list(count,6)=F1;
        if c==1
            grad=[t_G2(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5)),t_G3(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5)),t_G4(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5)),t_G5(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5)),t_G6(temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5))];
            p=grad./sqrt(grad(1)^2+grad(2)^2+grad(3)^2+grad(4)^2+grad(5)^2);
        end
        ...
        elseif next_x(3)<0
            ...
            F2=F(next_x(1),next_x(2),next_x(3),next_x(4),next_x(5));
        end
        if F2<F1
            temp_x=next_x;
            error=abs((F2-F1)/F1);
...
%             alph=5;
    ...
    final=[temp_x(1),temp_x(2),temp_x(3),temp_x(4),temp_x(5)];
    ...
    for i=j
        pre_test=CDF(final(1),final(2),final(3),i,final(4),final(5));
...
elseif ~isempty(fixa)&&(~isempty(fixb))&&isempty(fixtor)
    ...
    for i=1:length(data)
        t_G5=G5(fixa(1),fixa(2),fixb(1),fixb(2),data(i),tor1,tor2)+t_G5;
        t_G6=G6(fixa(1),fixa(2),fixb(1),fixb(2),data(i),tor1,tor2)+t_G6;        
        F=Lf(fixa(1),fixa(2),fixb(1),fixb(2),data(i),tor1,tor2)+F;
    end
    t_G5=matlabFunction(t_G5);
    t_G6=matlabFunction(t_G6);    
    F=matlabFunction(F);
    ...
    while error>criteria
        ...
        list(count,3)=F1;
        if c==1
            grad=[t_G5(temp_x(1),temp_x(2)),t_G6(temp_x(1),temp_x(2))];
...
%             alph=5;
    ...
    for i=j
        pre_test=CDF(fixa(1),fixa(2),fixb(1),fixb(2),i,final(1),final(2));
%         if pre_test>=low_limit&&pre_test<=high_limit
        test=[test,pre_test];
%         end
    end
