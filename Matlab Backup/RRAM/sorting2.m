function [probability,podes,sped,Incretemp,T_rate,J_rate]=sorting2(position,P_time,C_time)
...
elseif (~(p(1)<3||p(1)>4*n1-1||p(2)<3||p(2)>4*n1-1))&&(p(3)==1||p(3)==2)
    fod_tx=(Temd1(p(1)+2,p(2),p(3),C_time,C_temp)-Temd1(p(1)-2,p(2),p(3),C_time,C_temp))/(4*sqrt(2)*Unitcon);
    fod_ty=(Temd1(p(1),p(2)+2,p(3),C_time,C_temp)-Temd1(p(1),p(2)-2,p(3),C_time,C_temp))/(4*sqrt(2)*Unitcon);
    fod_tz=(Temd1(p(1),p(2),p(3)+2,C_time,C_temp)-Temd1(p(1),p(2),p(3),C_time,C_temp))/(2*Unitcon);
    th_direction=[fod_tx,fod_ty,fod_tz];
else
    fod_tx=(Temd1(p(1)+2,p(2),p(3),C_time,C_temp)-Temd1(p(1)-2,p(2),p(3),C_time,C_temp))/(4*sqrt(2)*Unitcon);
    fod_ty=(Temd1(p(1),p(2)+2,p(3),C_time,C_temp)-Temd1(p(1),p(2)-2,p(3),C_time,C_temp))/(4*sqrt(2)*Unitcon);
    fod_tz=(Temd1(p(1),p(2),p(3)+2,C_time,C_temp)-Temd1(p(1),p(2),p(3)-2,C_time,C_temp))/(2*Unitcon);
    th_direction=[fod_tx,fod_ty,fod_tz];
end
